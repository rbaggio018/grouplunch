When(/^(.*) makes an order (.*) which has price \$(.*)$/) do |user, order, price|
  visit root_url
...
Then(/^(.*) which has price \$(.*) under (.*) should be in the order list$/) do |order, price, user|
  visit orders_url
...
Then(/^(.*) which has price \$(.*) under (.*) should not be in the order list$/) do |order, price, user|
  visit orders_url
